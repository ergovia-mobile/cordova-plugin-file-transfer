...
public class FileTransfer extends CordovaPlugin {
    ...
    private static final String LOG_TAG = "FileTransferPortfolioTest";
 ...
 */
private void upload(final String source, final String target, JSONArray args, final CallbackContext callbackContext) throws JSONException {
...
private void chunkedUpload(final String source, final String target, JSONArray args, final CallbackContext callbackContext) throws JSONException {
            ...
            try {
                ...
                while (bytesRead > 0) {
                    ...
                    try {
                        ...
                        createBody(writer, sendStream, buffer, currentChunk, (int) totalChunks, mediaType, callbackContext);
                ...
                JSONObject error = createFileTransferError(LE_DEBUG, source, target, conn, iox);
                Log.e(LOG_TAG, error.toString(), iox);
                callbackContext.sendPluginResult(new PluginResult(PluginResult.Status.IO_EXCEPTION, error));

            } catch (JSONException json) {

                JSONObject error = createFileTransferError(LE_DEBUG, source, target, conn, json);
...
private void createBody(final PrintWriter writer, final OutputStream outputStream, final byte[] chunk, final int currentChunk, final int totalChunks, final int type, CallbackContext callbackContext) {
    ...
    addFilePart(writer, outputStream, "chunk", chunk, callbackContext);
}
...
private void addFilePart(final PrintWriter writer, final OutputStream outputStream, final String name, final byte[] chunk, CallbackContext callbackContext) {

    writer.append(LINE_START).append(BOUNDARY).append(LINE_END);
    writer.append("Content-Disposition: form-data; name=\"" + name +"\"").append(LINE_END);
